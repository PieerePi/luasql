...
#define LUASQL_ENVIRONMENT_DM "DM environment"
#define LUASQL_CONNECTION_DM "DM connection"
#define LUASQL_CURSOR_DM "DM cursor"
...
static env_data *getenvironment (lua_State *L) {
	env_data *env = (env_data *)luaL_checkudata (L, 1, LUASQL_ENVIRONMENT_DM);
...
static conn_data *getconnection (lua_State *L) {
	conn_data *conn = (conn_data *)luaL_checkudata (L, 1, LUASQL_CONNECTION_DM);
...
static cur_data *getcursor (lua_State *L) {
	cur_data *cur = (cur_data *)luaL_checkudata (L, 1, LUASQL_CURSOR_DM);
...
static int create_cursor (lua_State *L, int o, conn_data *conn, OCIStmt *stmt, const char *text) {
	...
	luasql_setmeta (L, LUASQL_CURSOR_DM);
	...
	/* DM and Lua column indices ranges from 1 to numcols */
...
static int conn_execute (lua_State *L) {
	...
	if (type == OCI_STMT_SELECT) {
		/* create cursor */
		// printf("type is OCI_STMT_SELECT\n");
		return create_cursor (L, 1, conn, stmthp, statement);
	} else {
		// printf("type is not OCI_STMT_SELECT\n");
...
static int env_connect (lua_State *L) {
	...
	/* fill in structure */
	luasql_setmeta (L, LUASQL_CONNECTION_DM);
	conn->env = LUA_NOREF;
	conn->closed = 1;
	conn->auto_commit = 1;
	conn->cur_counter = 0;
...
*/
	...
	OCIEnvInit(&env->envhp, OCI_DEFAULT,0, 0);
	...
	ASSERT (L, OCIHandleAlloc((dvoid *) env->envhp,
		(dvoid **) &(conn->srvhp),
		(ub4) OCI_HTYPE_SERVER, (size_t) 0, (dvoid **) 0), conn->errhp);
	// printf("srvhp %x.\n", conn->srvhp);
	/* session handler */
	ASSERT (L, OCIHandleAlloc((dvoid *) env->envhp,
		(dvoid **) &(conn->authp),
		(ub4) OCI_HTYPE_SESSION, (size_t) 0, (dvoid **) 0), conn->errhp);
	// printf("auth %x.\n", conn->authp);
	/* login */
	/* ASSERT (L, OCILogon(env->envhp, conn->errhp, &(conn->svchp),
		(CONST text*)username, userlen,
		...
		(CONST text*)sourcename, snlen), conn->errhp); */
	/* 连接服务器 */
	ASSERT (L, OCIServerAttach(conn->srvhp, conn->errhp, (text *)sourcename,
		(sb4)strlen(sourcename), OCI_DEFAULT), conn->errhp);
	/* 设置用户名和密码 */
	ASSERT (L, OCIAttrSet(conn->authp, OCI_HTYPE_SESSION, (text *)username,
		(ub4)strlen(username), OCI_ATTR_USERNAME, conn->errhp), conn->errhp);
	ASSERT (L, OCIAttrSet(conn->authp, OCI_HTYPE_SESSION, (text *)password,
		(ub4)strlen(password), OCI_ATTR_PASSWORD, conn->errhp), conn->errhp);
	/* 设置服务器环境句柄属性 */
	ASSERT (L, OCIAttrSet ((dvoid*)conn->svchp, (ub4) OCI_HTYPE_SVCCTX,
		(dvoid*)conn->srvhp, (ub4) 0, OCI_ATTR_SERVER, conn->errhp), conn->errhp);
	ASSERT (L, OCIAttrSet(conn->svchp, OCI_HTYPE_SVCCTX, (dvoid*) conn->authp,
		0, OCI_ATTR_SESSION, conn->errhp), conn->errhp);
	/* 创建并开始一个用户会话 */
	ASSERT (L, OCISessionBegin (conn->svchp, conn->errhp, conn->authp, OCI_CRED_RDBMS, OCI_DEFAULT), conn->errhp);
#endif
...
static int create_environment (lua_State *L) {
	...
	luasql_setmeta (L, LUASQL_ENVIRONMENT_DM);
	/* fill in structure */
	env->closed = 0;
	env->conn_counter = 0;
	env->envhp = NULL;
...
static void create_metatables (lua_State *L) {
	...
	luasql_createmeta (L, LUASQL_ENVIRONMENT_DM, environment_methods);
	luasql_createmeta (L, LUASQL_CONNECTION_DM, connection_methods);
	luasql_createmeta (L, LUASQL_CURSOR_DM, cursor_methods);
	lua_pop (L, 3);
}
...
** driver open method.
*/
LUASQL_API int luaopen_luasql_dm (lua_State *L) {
	struct luaL_Reg driver[] = {
		{"dm", create_environment},
		{NULL, NULL},
	};
	create_metatables (L);
	lua_newtable (L);
